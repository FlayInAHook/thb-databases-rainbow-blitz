...
export async function registerUser(username: string, uniqueID: string) {
  const user = new User({username, levelData: {}, uniqueID, isDev: false});
  return await user.save();
}

export async function registerCompleteUser(registeringUser: TUser) {
  const user = new User(registeringUser);
  return await user.save();
}
